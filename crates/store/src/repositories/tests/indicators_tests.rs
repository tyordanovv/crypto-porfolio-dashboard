use crate::{models::indicator_db::IndicatorDB, repositories::indicator_repository::IndicatorRepo};

use super::establish_test_pool;
use chrono::NaiveDate;

fn create_record(date: (i32, u32, u32), value: f64) -> IndicatorDB {
    IndicatorDB {
        name: "RSI".to_string(),
...
fn test_insert_indicator() {
    ...
    let inserted = IndicatorRepo::insert(&mut conn, &rec).expect("insert failed");
...
fn test_latest_indicator() {
    ...
    IndicatorRepo::insert(&mut conn, &rec1).unwrap();
    IndicatorRepo::insert(&mut conn, &rec2).unwrap();

    let latest = IndicatorRepo::latest(&mut conn, "RSI").unwrap().unwrap();
...
fn test_range_indicators() {
    ...
    let range = IndicatorRepo::range(&mut conn, "RSI", rec1.timestamp, rec2.timestamp).unwrap();
...
fn test_upsert_indicator() {
    ...
    let rec = create_record((2024, 10, 1), 55.3);
    IndicatorRepo::insert(&mut conn, &rec).unwrap();

    // update the value for the same timestamp
    let updated = create_record((2024, 10, 1), 58.0);
    IndicatorRepo::insert(&mut conn, &updated).unwrap();

    let fetched = IndicatorRepo::range(&mut conn, "RSI", rec.timestamp, rec.timestamp).unwrap();
