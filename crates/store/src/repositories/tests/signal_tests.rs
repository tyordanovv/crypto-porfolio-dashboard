use crate::{models::signal_db::{NewStrategySignalDB, StrategySignalDB}, repositories::signal_repository::SignalsRepo};

use super::establish_test_pool;

fn create_signal(asset: &str, signal_type: &str, value: f64) -> NewStrategySignalDB {
    NewStrategySignalDB {
...
fn test_upsert_signal() {
    ...
    let upsert_rec = StrategySignalDB {
        timestamp: fetched.timestamp, // same timestamp
        ..fetched
    };

    let upsert_insertable = NewStrategySignalDB {
