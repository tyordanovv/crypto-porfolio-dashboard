...
use crate::models::{ StrategySignal, NewStrategySignal };
...
impl SignalsRepo {
    pub fn insert(conn: &mut PgPooledConnection, rec: &NewStrategySignal) -> Result<usize, DieselError> {
    ...
    pub fn latest_for_asset_and_type(
        ...
        asset: &str,
        signal_type_str: &str,
    ) -> Result<Option<StrategySignal>, DieselError> {
    ...
    pub fn get_between(
        ...
        from: NaiveDateTime,
        to: NaiveDateTime,
    ) -> Result<Vec<StrategySignal>, DieselError> {
        strategy_signals::table
            .filter(strategy_signals::asset_symbol.eq(asset))
            .filter(strategy_signals::signal_type.eq(signal_type_str))
